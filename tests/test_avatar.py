...
class TestAvatar:
    ...
    def test_change_ava(self, revert_avatar, login):
        ...
        # После полной загрузки аватара, открываем редактирование аватара
        WebDriverWait(driver, 20).until(
            EC.presence_of_element_located(Locators.PROFILE_IMAGE)
        ).click()

        # Ждем пока появится поле ввода URL аватара
        ...
        update_button = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable(Locators.AVATAR_BUTTON)
        )
        update_button.click()
